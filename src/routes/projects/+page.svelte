...
<section class="projects">
    ...
    {:else}
        <div class="hovered-temp">
            List of projects that I've developed
        </div>
    {/if}
    <div class="box-piles">
        <div class="pile">
            {#each pile1 as box, index}
                {#if ready}
                    <div
                        ...
                        in:fly={{ delay: index*50, duration: index*50 + 400, y: -50, opacity: 1, easing: heavyBounceOut }}
                    >
                        <a href={box.url} target="_blank"><Package icon={box.icon} size={box.size} shadow={index==0}/></a>
                    </div>
                {/if}
            {/each}
        </div>
            ...
            {#each pile2 as box, index}
                {#if ready}
                    <div
                        ...
                        on:mouseenter={() => (hoveredBox = box)}
                        on:mouseleave={() => (hoveredBox = null)}
                        in:fly={{ delay: index*60, duration: index*50 + 400, y: -50, opacity: 1, easing: heavyBounceOut }}
                    >
                        <a href={box.url} target="_blank"><Package icon={box.icon} size={box.size} shadow={index==0}/></a>
                    </div>
                {/if}
            ...
            {#each pile3 as box, index}
                {#if ready}
                    <div
                        ...
                        on:mouseenter={() => (hoveredBox = box)}
                        on:mouseleave={() => (hoveredBox = null)}
                        in:fly={{ delay: index*80, duration: index*50 + 400, y: -50, opacity: 1, easing: heavyBounceOut }}
                    >
                        <a href={box.url} target="_blank"><Package icon={box.icon} size={box.size} shadow={index==0}/></a>
                    </div>
                {/if}
